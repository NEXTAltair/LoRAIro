...
Serena(高速・直接操作)とMoltbot LTM(長期記憶・設計知識)の適切な使い分けを案内。
...
2. Moltbot LTM活用 - 設計方針、過去の設計変更結果参照
...
def generate_guidance_message(file_path: str, file_category: str) -> str:
    ...
    ltm_guidance = (
        "🧠 **Moltbot LTM活用推奨** (設計・歴史的理解):\n"
        "• **設計方針が必要な時**: `python3 .github/skills/lorairo-mem/scripts/ltm_search.py \"設計方針\"`\n"
        "• **過去の変更理由が必要な時**: LTM検索で過去の設計変更結果を参照\n"
        "• **複合的判断が必要な時**: Serena + LTM を組み合わせた包括的分析\n\n"
    )
...
**高度な分析が必要な場合:**
• `python3 .github/skills/lorairo-mem/scripts/ltm_search.py "{pattern}"` - LTM検索
