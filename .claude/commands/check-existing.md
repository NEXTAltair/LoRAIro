---
allowed-tools: Read, Glob, Grep, Bash, TodoWrite, WebSearch, WebFetch, Task
...
- `docs/decisions/` で過去の設計判断を確認
- `docs/lessons-learned.md` で関連するバグパターン・教訓を確認
- OpenClaw LTM で過去の調査記録を検索
...
- **アーキテクチャ適合**: `Glob`/`Grep` で統合箇所のコード分析
...
#### 5.1 知識蓄積
調査結果と判断を長期記憶として保存（OpenClaw LTM経由でNotionに保存）
