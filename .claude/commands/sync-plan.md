...
- 実装後、重要な設計決定を OpenClaw LTM に抽出することを推奨
```
...
1. **Serena Memory のみ**:
   ...
   - OpenClaw LTM（長期、クロスプロジェクト）への保存は手動で実行
...
- `/planning`: 包括的な設計フェーズ（OpenClaw LTM に直接保存）
...
4. 実装完了後、重要な設計決定を OpenClaw LTM に抽出
