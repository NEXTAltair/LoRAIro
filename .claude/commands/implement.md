...
planフェーズで策定された実装計画に基づき、LoRAIro プロジェクトの実際のコード実装を行います。
...
planフェーズで承認された設計に基づき、上記実装対象にLoRAIroの確立されたアーキテクチャパターンに従って高品質なコードを実装します。テスト駆動開発とコード品質維持を重視した段階的実装を行います。

## タスクに含まれるべきTODO

### 1. 実装前チェックリスト
1. planフェーズの実装計画を詳細確認
2. 要件・設計の完全理解確認
...
## 出力形式
1. **ultrathink実装プロセス**: 実装アプローチと思考過程
