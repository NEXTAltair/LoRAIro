...
def test_production_integration():
    ...
    print(
        f"1. ServiceContainer動作モード: {'プロダクション' if container.is_production_mode() else 'Mock'}"
    )
    ...
    try:
        ...
        results = adapter.call_annotate(
            images=[test_image], models=test_models, phash_list=["test_hash_001"]
        )
                ...
                if result.get("error"):
                    ...
                    if result.get("formatted_output"):
                        ...
                        if isinstance(formatted, dict):
                            # キャプション表示
                            if formatted.get("captions"):
                                caption = (
                                    formatted["captions"][0][:100] + "..."
                                    if len(formatted["captions"][0]) > 100
                                    else formatted["captions"][0]
                                )
