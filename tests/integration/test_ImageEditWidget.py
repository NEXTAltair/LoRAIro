...
from gui.window.edit import ImageEditWidget
...
def test_initialization(
    app, mock_config_manager, mock_file_system_manager, mock_image_database_manager, mock_main_window
):
    widget = ImageEditWidget()
    widget.initialize(mock_config_manager, mock_file_system_manager, mock_image_database_manager, mock_main_window)
    ...
    assert [widget.comboBoxUpscaler.itemText(i) for i in range(4)] == [
        "None",
        "RealESRGAN_x4plus",
        "Lanczos",
        "Bicubic",
...
def test_process_all_images(widget, mocker):
    ...
    mock_get_existing_annotations = mocker.patch(
        "src.ImageEditWidget.ImageAnalyzer.get_existing_annotations", return_value={"tags": [], "captions": []}
    )
    ...
    widget.process_all_images(
        progress_callback=progress_callback, status_callback=status_callback, is_canceled=is_canceled
    )
...
def test_on_pushButtonStartProcess_clicked(widget, mocker):
    ...
    mock_some_long_process = mocker.patch.object(
        widget.main_window, "some_long_process", side_effect=lambda func: func()
    )
