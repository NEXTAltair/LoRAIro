...
""")
        ...
        with patch('lorairo.services.configuration_service.get_config') as mock_get_config:
            mock_get_config.return_value = {
                "directories": {"database_dir": str(project_dir)}
            }
    ...
    def test_overview_widget_path_resolution_integration(self, tmp_path):
        ...
        image_paths = []
        for i in range(3):
            subdir = project_dir / "image_dataset" / "original_images" / "2024" / "10" / f"0{i+1}" / f"source_{i}"
            subdir.mkdir(parents=True)
            image_file = subdir / f"image_{i:03d}.jpg"
            image_file.write_bytes(f"fake_image_data_{i}".encode())
            image_paths.append(f"image_dataset/original_images/2024/10/0{i+1}/source_{i}/image_{i:03d}.jpg")
        ...
        absolute_image_paths = [
            database_dir / Path(item["stored_image_path"]) 
        ...
        image_metadata_map = {
            item["id"]: {
                "path": database_dir / Path(item["stored_image_path"]), 
                "metadata": item
            }
            for item in filtered_image_metadata
        }
    ...
    def test_thumbnail_loading_path_integration(self, tmp_path):
        ...
        # データベースの相対パス形式
        relative_paths = [
            f"image_dataset/original_images/test_{i}.jpg" 
            for i in range(2)
        ]
    ...
    def test_missing_file_handling_integration(self, tmp_path):
        ...
        missing_absolute_paths = [
            database_dir / Path(rel_path) 
            for rel_path in missing_relative_paths
        ]
    ...
    def test_unicode_filename_path_resolution(self, tmp_path):
        ...
        # 相対パスでの表現
        relative_paths = [
            f"image_dataset/original_images/2024/猫画像/{filename}"
            for filename in unicode_files
        ]
