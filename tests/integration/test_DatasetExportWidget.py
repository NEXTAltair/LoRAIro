...
from gui.window.export import DatasetExportWidget
...
def test_on_filter_applied(qtbot, mock_config_manager, mock_file_system_manager, mock_image_database_manager, mocker):
    # テストケースを定義
    test_cases = [
        {
    ...
    for case in test_cases:
        ...
        if case["expect_no_results"]:
            ...
            mock_update_thumbnail_selector.assert_called_once_with(
                list(expected_image_path_id_map.keys()), case["list_count"]
            )
...
def test_on_exportButton_clicked_no_export_directory(
    qtbot, mock_config_manager, mock_file_system_manager, mock_image_database_manager, mocker
):
    widget = DatasetExportWidget()
    widget.initialize(mock_config_manager, mock_file_system_manager, mock_image_database_manager)
    qtbot.addWidget(widget)
...
def test_on_exportButton_clicked_no_export_formats(
    qtbot, mock_config_manager, mock_file_system_manager, mock_image_database_manager, mocker
):
    widget = DatasetExportWidget()
    widget.initialize(mock_config_manager, mock_file_system_manager, mock_image_database_manager)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {Path("/path/to/image1.jpg"): 1, Path("/path/to/image2.jpg"): 2},
            [
...
def test_export_dataset(
    ...
    if not expect_warning and not expect_critical:
        ...
        assert mock_file_system_manager.export_dataset_to_txt.call_count == expected_call_count * (
            1 if "txt_cap" in formats else 0
        )
        assert mock_file_system_manager.export_dataset_to_json.call_count == expected_call_count * (
            1 if "json" in formats else 0
        )
    else:
...
def test_update_thumbnail_selector(
    qtbot, mock_config_manager, mock_file_system_manager, mock_image_database_manager, mocker
):
    ...
    widget.thumbnailSelector.get_selected_images.return_value = [
        Path("/path/to/image1.jpg"),
