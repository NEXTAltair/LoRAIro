...
class TestMainWindowBatchIntegration:
    ...
    def test_dataset_directory_change_triggers_batch_processing(
        self, configured_main_window, test_images_directory
    ):
    ...
    def test_batch_processing_workflow_integration(self, configured_main_window, test_images_directory):
        ...
        window.idm.register_original_image.side_effect = lambda file, fsm: (
            f"id_{file.stem}",
            {"test": "data"},
        )
    ...
    def test_batch_progress_display_integration(self, configured_main_window):
        ...
        # Assert
        window.statusbar.showMessage.assert_called_once_with("処理中: test_image.jpg (3/10)")
        window.progress_widget.setWindowTitle.assert_called_once_with("バッチ処理 - 30% 完了")

    def test_batch_processing_error_handling_integration(
        self, configured_main_window, test_images_directory
    ):
        ...
        with (
            patch.object(window, "some_long_process", side_effect=mock_some_long_process_with_error),
    ...
    def test_end_to_end_batch_processing_simulation(self, configured_main_window, test_images_directory):
        ...
        with patch.object(window, "some_long_process") as mock_long_process:
            mock_long_process.side_effect = lambda func, *args, **kwargs: simulate_batch_processing(
                *args, **kwargs
            )
...
class TestMainWindowRealComponentIntegration:
    ...
    def test_progress_widget_controller_integration(self):
        ...
        with patch.object(MainWindow, "__init__", return_value=None):
            ...
            from lorairo.gui.window.progress import Controller, ProgressWidget
...
class TestMainWindowUserInteractionSimulation:
    ...
    def test_error_recovery_workflow(self, configured_main_window, tmp_path):
        ...
        with (
            patch.object(window, "some_long_process", side_effect=alternating_process),
            patch("lorairo.gui.window.main_window.logger") as mock_logger,
        ):
            # Act - エラー発生
