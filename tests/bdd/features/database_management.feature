...
Feature: 画像データベース管理機能
  ...
  Scenario: 同一画像の再登録は pHash 重複として既存画像にスキップされる
    Given オリジナル画像が登録されている
    When 同じ画像をもう一度登録する
    Then 再登録結果の画像IDは最初の登録と一致する
    And データベースの画像件数は1件のままである

  Scenario: 同一pHashでもグレースケール版は別版として新規登録される
    Given オリジナル画像が登録されている
    When 同じpHashのグレースケール版を登録する
    Then 別版登録結果の画像IDは最初の登録と異なる
    And データベースの画像件数は2件になる
