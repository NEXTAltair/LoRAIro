...
from lorairo.editor.image_processor import ImageProcessor, ImageProcessingManager
...
class TestImageProcessingManager:
        ...
        try:
            ...
            with patch('lorairo.editor.image_processor.AutoCrop') as mock_autocrop:
                ...
                result = self.manager.process_image(
                    db_stored_original_path=tmp_path,
    ...
    def test_process_image_with_nonexistent_file(self):
        ...
        result = self.manager.process_image(
            db_stored_original_path=non_existent_path,
            original_has_alpha=False,
            original_mode='RGB'
        )
    ...
    def test_initialization_with_invalid_parameters(self):
        ...
        # Test with invalid preferred_resolutions that cause an error during initialization
        with pytest.raises(ValueError, match="ImageProcessingManagerの初期化中エラー"):
            # Pass invalid preferred_resolutions type
            ImageProcessingManager(
                self.mock_file_system,
                target_resolution=512,
                preferred_resolutions=None  # Invalid type
            )
