...
from editor.image_processor import ImageProcessingManager, ImageProcessor, AutoCrop, Upscaler
from unittest.mock import MagicMock


@pytest.mark.parametrize(
    "image_type, has_alpha, mode",
    [("rgb", False, "RGB"), ("rgba", True, "RGBA"), ("cmyk", False, "CMYK"), ("p", False, "P")],
)
def test_normalize_color_profile(
    mock_file_system_manager, sample_images, preferred_resolutions, image_type, has_alpha, mode
):
    processor = ImageProcessor(
        mock_file_system_manager, target_resolution=1024, preferred_resolutions=preferred_resolutions
    )
...
def test_resize_image_with_matching_resolution(mock_file_system_manager, sample_images, preferred_resolutions):
    processor = ImageProcessor(
        mock_file_system_manager, target_resolution=512, preferred_resolutions=preferred_resolutions
    )
...
def test_process_image_rgb_no_alpha(mock_file_system_manager, sample_images, preferred_resolutions):
    manager = ImageProcessingManager(
        file_system_manager=mock_file_system_manager, target_resolution=512, preferred_resolutions=preferred_resolutions
    )
    # モックファイルシステムに画像を配置
    ...
    resized_image = manager.process_image(
        db_stored_original_path=original_path, original_has_alpha=False, original_mode="RGB", upscaler=None
    )
...
def test_process_image_rgba_with_alpha(mock_file_system_manager, sample_images, preferred_resolutions):
    ...
    resized_image = manager.process_image(
        db_stored_original_path=original_path, original_has_alpha=True, original_mode="RGBA", upscaler=None
    )
