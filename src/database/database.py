...
class ImageRepository:
    ...
    def add_original_image(self, info: dict[str, Any]) -> int:
        ...
        try:
            ...
            self.logger.error(
                f"{self.__class__.__name__}.{self.add_original_image.__name__} - SQL エラーが 発生しました: {e}"
            )
            raise
    ...
    def save_annotations(self, image_id: int, annotations: dict[str, Union[list[str], float, int]]) -> None:
        ...
        if model_id is None:
            model_id = next(
                (tag.get("model_id") for tag in tags if tag.get("model_id") is not None),
                None,
            )
            if model_id is None:
                model_id = next(
...
class ImageDatabaseManager:
    ...
    def _filter_images_by_exclude_keywords(self, image_ids: set[int], exclude_keywords: list[str]) -> set[int]:
        ...
        for image_id in image_ids_copy:
            ...
            exclude_match = any(
                keyword.lower() in annotation for keyword in exclude_keywords for annotation in annotations
            )
    ...
    def check_processed_image_exists(self, image_id: int, target_resolution: int) -> Optional[dict]:
        ...
        try:
            ...
            self.logger.info(
                f"ID {image_id} の画像に解像度 {target_resolution} に一致する処理済み画像が見つかりませんでした"
            )
            return None
        except Exception as e:
            current_method = inspect.currentframe().f_code.co_name
    ...
    def filter_recent_annotations(self, annotations: dict) -> dict:
        ...
        self.logger.debug(
            f"最新の更新時刻 {latest_datetime} から {time_threshold} 以内に更新されたアノテーションをエクスポート"
        )
        ...
        return {"tags": filtered_tags, "captions": filtered_captions}
