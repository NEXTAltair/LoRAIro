...
class _ExactMatchEntry(NamedTuple):
    ...
    翻訳品質評価とタグレコード品質評価が同一の search 結果を共有し、per-tag の
    search_tags を重複させない (#998) ための束ね。

    Attributes:
        translations: 言語別翻訳候補。完全一致行が無ければ None。
        tag_ids: 完全一致行の tag_id リスト (user overlay 照合用)。
        row: tag-record refinement に渡す完全一致行 (最初の1件)。無ければ None。
    """

    translations: dict[str, list[str]] | None
    tag_ids: list[int]
    row: TagRecordPublic | None
...
class TagManagementService:
    ...
    # tag-record refinement (recommend_tag_record_refinement) の reason のうち、翻訳の管理
    # フローで ⚠ 表示する code を明示選別する (#1123、#993 の教訓に倣った allowlist)。
    # status/type 起因のレコード品質問題に限定し、proposal 候補寄りの status_type_conflict /
    # type_correction_candidate は ⚠ ノイズを避けるため表示対象から除外する (proposals には残る)。
    _TAG_RECORD_REASON_CODES = frozenset(
        {
            "deprecated_tag",
            "unknown_type",
    ...
    @property
    def _translation_prefetch_cache(self) -> dict[str, _ExactMatchEntry]:
    ...
    def recommend_with_translation_quality(
        ...
        Returns:
            manual refinement reason・翻訳品質 reason・タグレコード品質 reason を統合した
            RefinementRecommendation (#976/#1123)。
        """
        ...
        # 無し) と manual が判定した場合、グローバルに引いた翻訳やレコードの評価は無関係な偽陽性 ⚠
        # になるためスキップする (#993、_SKIP_TRANSLATION_REASON_CODES の docstring 参照)。
        if self._SKIP_TRANSLATION_REASON_CODES.intersection(reason.code for reason in manual.reasons):
            return manual
        # 完全一致行を 1 回だけ取得し、翻訳品質評価とタグレコード品質評価で共有する (#998 N+1 回避)。
        entry = self._fetch_exact_match_entry(tag, repo=repo)
        extra: list[RefinementRecommendation] = []
        # #1123: タグレコード (status/type) 品質提案を合流させる
        record_rec = self._recommend_tag_record(entry.row, format_name=format_name, repo=repo)
        if record_rec is not None:
            extra.append(record_rec)
        extra.extend(self._evaluate_translation_quality_from_entry(tag, entry, repo=repo))
        if not extra:
            return manual
        return self._merge_recommendations(manual, extra)

    def _recommend_tag_record(
        self, row: TagRecordPublic | None, *, format_name: str, repo: object | None = None
    ) -> RefinementRecommendation | None:
        """タグレコード (status/type) 品質提案を評価し、表示対象 reason だけ返す (#1123)。

        genai-tag-db-tools の `recommend_tag_record_refinement` に overlay 合成済みの検索行を
        渡し、deprecated type / unknown type / missing_format_status 等を DB 無変更で検出する。
        表示対象 code は `_TAG_RECORD_REASON_CODES` で明示選別する (#993 の教訓)。取得失敗や
        表示対象 reason が無い場合は None を返す (非ブロッキング)。

        Args:
            row: 完全一致行 (tag-record refinement の入力)。None なら評価しない。
            format_name: タグの format 名。判定不能時は "unknown"。
            repo: lib に渡す DB リーダー。None なら merged reader を使う。

        Returns:
            表示対象 reason に絞った RefinementRecommendation。無ければ None。
        """
        if row is None:
            return None
        reader = repo if repo is not None else self.reader
        try:
            rec = recommend_tag_record_refinement(row, format_name=format_name, repo=reader)
        except (ValueError, RuntimeError, SQLAlchemyError) as e:
            logger.warning(f"タグレコード品質評価に失敗 (スキップ): tag='{row.tag}': {e}")
            return None
        if not rec.needs_refinement:
            return None
        kept = [reason for reason in rec.reasons if reason.code in self._TAG_RECORD_REASON_CODES]
        if not kept:
            return None
        # 表示対象に紐づく proposals のみ残す (ignore フィルタと同方針)。
        kept_codes = {reason.code for reason in kept}
        kept_proposals = [
            proposal
            for proposal in rec.proposals
            if not proposal.reason_codes or (set(proposal.reason_codes) & kept_codes)
        ]
        return rec.model_copy(
            update={"needs_refinement": True, "reasons": kept, "proposals": kept_proposals}
        )

    def _evaluate_translation_quality(
        self, tag: str, *, repo: object | None = None
    ) -> list[RefinementRecommendation]:
        """タグの ja 翻訳品質を評価し、問題が出たものだけ返す (#976)。互換用の取得つき窓口。"""
        entry = self._fetch_exact_match_entry(tag, repo=repo)
        return self._evaluate_translation_quality_from_entry(tag, entry, repo=repo)

    def _evaluate_translation_quality_from_entry(
        self, tag: str, entry: _ExactMatchEntry, *, repo: object | None = None
    ) -> list[RefinementRecommendation]:
        """取得済みの完全一致行 (`entry`) から ja 翻訳品質を評価する (#976/#1123)。
    ...
    def _fetch_exact_match_translations(
        ...
        return self._fetch_exact_match_entry(tag, repo=repo).translations

    def _fetch_exact_match_entry(self, tag: str, *, repo: object | None = None) -> _ExactMatchEntry:
        """入力タグに完全一致する行の翻訳・tag_id・レコードを取得する (#976/#1123)。
        ...
        Returns:
            _ExactMatchEntry(翻訳マップ, tag_id リスト, 完全一致行)。完全一致行が無ければ
            (None, [], None)。tag_id は user overlay の provenance 照合 (#1054)、row は
            tag-record refinement (#1123) に使う。
        """
        cached = self._translation_prefetch_cache.pop(tag.casefold(), _PREFETCH_MISS)
        if cached is not _PREFETCH_MISS:
            return cast("_ExactMatchEntry", cached)
    ...
    def _extract_exact_entry(self, result: TagSearchResult, tag: str) -> _ExactMatchEntry:
        """search 結果から入力タグに完全一致する行の翻訳・tag_id・レコードを抽出する (#976/#998/#1123)。
    ...
    def prefetch_translations(self, tags: Iterable[str], *, repo: object | None = None) -> None:
        ...
        for tag in unique:
            ...
            if result is None:
                # マッチ無し = 完全一致行なし → (None, [], None) (呼び出し元は素通し)。
                cache[tag.casefold()] = _ExactMatchEntry(None, [], None)
            else:
    ...
    def list_translation_candidates(self, canonical: str, language: str) -> tuple[list[str], str | None]:
        ...
        exact_entry = self._extract_exact_entry(result, canonical)
        translations_map, tag_ids = exact_entry.translations, exact_entry.tag_ids
        if not tag_ids:
            return [], None
        tag_id = tag_ids[0]
