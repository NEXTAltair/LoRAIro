...
class UIResponsiveConversionService:
    ...
    def _initialize_responsive_patterns(self) -> dict[str, ResponsivePattern]:
        ...
        return {
            ...
            "content_areas": ResponsivePattern(
                ...
                conversion_rules={"hsizetype": "Expanding", "vsizetype": "Expanding"},
                validation_criteria={"required_hsizetype": "Expanding", "required_vsizetype": "Expanding"},
            ),
            # ダイアログボタン群 - 水平拡張、垂直固定
            "dialog_buttons": ResponsivePattern(
                ...
                validation_criteria={"required_hsizetype": "Expanding", "required_vsizetype": "Fixed"},
            ),
            # 入力フィールド - 水平拡張、垂直優先
            "input_fields": ResponsivePattern(
                ...
                validation_criteria={"required_hsizetype": "Expanding", "required_vsizetype": "Preferred"},
            ),
            # 表示専用ラベル - 水平優先、垂直固定
            "display_labels": ResponsivePattern(
                widget_types=["QLabel"],
                target_properties=["sizePolicy"],
                conversion_rules={"hsizetype": "Preferred", "vsizetype": "Fixed"},
                validation_criteria={"required_hsizetype": "Preferred", "required_vsizetype": "Fixed"},
            ),
            # コンテナフレーム - コンテキスト依存
            ...
            "horizontal_layouts": ResponsivePattern(
                ...
                validation_criteria={"max_margin": "5", "max_spacing": "15"},
            ),
            # 垂直レイアウト - 構造的スペーシング
            "vertical_layouts": ResponsivePattern(
                ...
                validation_criteria={"max_margin": "15", "max_spacing": "20"},
            ),
            # プログレスバー - 全幅拡張
    ...
    def convert_ui_files(self, target_files: list[str] | None = None) -> list[ConversionResult]:
        """UIファイル群のレスポンシブ変換実行
        ...
        for file_name in target_files:
            ...
            try:
                ...
                results.append(
                    ConversionResult(
                        file_path=file_path, success=False, changes_made=0, error_message=str(e)
                    )
                )
    ...
    def _convert_single_ui_file(self, file_path: Path) -> ConversionResult:
        ...
        return ConversionResult(
            file_path=file_path, success=True, changes_made=changes_made, backup_path=backup_path
        )
        ...
        for widget_type in adjusted_pattern.widget_types:
            ...
            if widget_type.endswith("Layout"):
                ...
                for layout in layouts:
                    if self._should_apply_pattern_to_layout(layout, adjusted_pattern, root):
    ...
    def _analyze_dialog_context(self, root: ET.Element) -> dict[str, str]:
        ...
        return {"type": dialog_type, "category": category, "class_name": class_name}

    def _adjust_pattern_for_context(
        self, pattern: ResponsivePattern, context: dict[str, str]
    ) -> ResponsivePattern:
        """コンテキスト依存パターン調整
    ...
    def _should_apply_pattern_to_widget(
        self, widget: ET.Element, pattern: ResponsivePattern, context: dict[str, str]
    ) -> bool:
        """ウィジェット適用判定
        ...
        if current_hsizetype == target_hsizetype and current_vsizetype == target_vsizetype:
            return False
    ...
    def _should_apply_pattern_to_layout(
        self, layout: ET.Element, pattern: ResponsivePattern, root: ET.Element
    ) -> bool:
        """レイアウト適用判定
        ...
        Args:
            layout: 対象レイアウト要素
            pattern: 変換パターン
            root: XML ルート要素（ネストレベル計算用）

        Returns:
            適用可能フラグ
        """
        # ネストレベルが深すぎる場合は適用しない
        nesting_level = self._calculate_layout_nesting_level(layout, root)
        if nesting_level > 4:
            return False
    ...
    def _calculate_layout_nesting_level(self, layout: ET.Element, root: ET.Element) -> int:
        """レイアウトネストレベル計算

        標準 xml.etree.ElementTree で親参照を得るために親マップを構築して使用する。

        Args:
            layout: 対象レイアウト要素
            root: XML ルート要素（親マップ構築用）

        Returns:
            ネストレベル (0が最上位)
        """
        # 標準 ElementTree は親参照を持たないため親マップを構築
        parent_map = {child: parent for parent in root.iter() for child in parent}
        level = 0
        current: ET.Element = layout
    ...
    def _get_layout_property_value(self, layout: ET.Element, property_name: str) -> str | None:
        """レイアウトプロパティ値取得
    ...
    def _find_widgets_by_criteria(
        ...
        # プロパティフィルタリング
        if has_property:
            widgets = [w for w in widgets if w.find(f".//property[@name='{has_property}']") is not None]
    ...
    def _preserve_xml_formatting(self, tree: ET.ElementTree[ET.Element], file_path: Path) -> None:
        """XMLフォーマット保持でファイル保存
        ...
        if root_elem is not None:
            self._indent_xml_element(root_elem)
    ...
    def _check_size_policy_issues(self, root: ET.Element) -> list[dict[str, str | None]]:
        """sizePolicyの問題チェック
    ...
    def _get_widget_size_policy(self, widget: ET.Element) -> dict[str, str | None]:
        """ウィジェットのsizePolicy取得
        ...
        for result in results:
            if result.success and result.file_path.exists():
                try:
                    ...
                    compatibility["incompatible_files"] += 1
                    compatibility["compatibility_issues"].append(
                        {"file": result.file_path.name, "issue": "xml_parse_error", "error": str(e)}
                    )
                ...
                compliance["compliance_details"].append(
                    {
                        ...
                        "issues_count": len(file_validation["size_policy_issues"])
                        + len(file_validation["layout_issues"]),
                    }
                )
