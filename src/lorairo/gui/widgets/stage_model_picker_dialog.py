...
データが無いため描画しない。conf-min 閾値 UI も持たない: lib はタグ閾値を推論時に
静的 ``tag_threshold`` で適用して confidence を捨てるため、推論後に閾値を当てる対象が
存在しない (#851 で撤去)。実行時 per-model 閾値が要るなら lib API 拡張が前提になる。
"""
...
class _ModelRow(QFrame):
    """モデル候補 1 件分の行ウィジェット (チェック + メタ + status + コスト)。
