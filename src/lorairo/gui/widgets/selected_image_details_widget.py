...
class SelectedImageDetailsWidget(QWidget):
    ...
    def __init__(
        ...
        # 翻訳候補の非同期取得 (#1232)。翻訳管理ポップアップの候補読み込みを worker へ退避し、
        # GUI スレッドのブロック (約1秒) を解消する。single-flight (言語切替で置換)。
        self._trans_candidates_generation: int = 0
        self._trans_candidates_inflight_id: str | None = None
        self._trans_candidates_callback: Callable[[list[str], str | None], None] | None = None
        ...
        self._user_db_poll_timer.timeout.connect(self._poll_user_db_change)
        self._user_db_poll_timer.start()
    ...
    def set_tag_management_service(self, service: "TagManagementService") -> None:
        ...
        # 候補取得を worker へ退避する非同期 provider も注入する (#1232)。注入があると
        # ダイアログは同期 provider を使わず、ポップアップ表示がブロックしない。
    ...
    def _start_translation_candidates_fetch(
        ...
        canonical: str,
        language: str,
        on_result: Callable[[list[str], str | None], None],
    ) -> None:
        """翻訳候補を worker で非同期取得し、完了時に on_result へ渡す (#1232)。

        翻訳管理ダイアログが候補を要求するたびに呼ばれる。実行中の候補 worker は 1 本に
        制限し (single-flight)、言語切替による新要求は旧 worker を協調キャンセルして最新
        だけを反映する。service 未配線なら即座に空候補で応答する。

        Args:
            canonical: 候補を列挙する canonical タグ。
            language: 言語コード ("ja" / "en")。
            on_result: 完了時に GUI スレッドで ``(候補訳, 主訳)`` を渡すコールバック。
        """
        service = self._tag_management_service
        if service is None:
            on_result([], None)
        ...
        from ..workers.translation_candidates_worker import TranslationCandidatesWorker

        self._trans_candidates_generation += 1
        generation = self._trans_candidates_generation
        self._trans_candidates_callback = on_result
        if self._trans_candidates_inflight_id is not None:
            from ..workers.terminal import CancelReason

            manager.request_cancel_worker(
                self._trans_candidates_inflight_id, reason=CancelReason.SEARCH_REPLACED
            )
    ...
    def _on_tag_metadata_terminal(self, event: "WorkerTerminalEvent") -> None:
        ...
        # worker_terminal シグナル配送中に start_worker (QThread 生成/connect) を同期実行すると、
        # 旧 worker の teardown と競合してメインスレッドが恒久ブロックし得る (#1206 py-spy 実測)。
        # イベントループ 1 巡後に遅延起動し、シグナル配送コンテキストから切り離す。
        QTimer.singleShot(0, lambda: self._start_tag_metadata_worker_deferred(pending))

    def _start_tag_metadata_worker_deferred(self, request: tuple[int, list[dict[str, Any]], int]) -> None:
        """遅延起動された pending tag metadata 要求を、発火時点の状態で再検証して起動する (#1206)。

        terminal イベントから発火までの間に shutdown / 新 worker 起動 / 画像切替が
        起こり得るため、terminal ハンドラでの検証とは別に発火時点で再検証する。

        Args:
            request: (image_id, tags_list, generation) の解決要求。
        """
        if self._closing or self._tag_metadata_inflight_id is not None:
            return  # shutdown 済み、または新しい要求が既に起動済み → stale pending は破棄
        if request[0] != self.current_image_id:
            return
        self._start_tag_metadata_worker(request)
    ...
    def _on_refinement_terminal(self, event: "WorkerTerminalEvent") -> None:
        ...
        if pending[0] != self.current_image_id or not self._current_tag_canonicals:
            return
        # tag metadata と同様、シグナル配送中の同期 start_worker を避け遅延起動する (#1206)。
        QTimer.singleShot(0, lambda: self._start_refinement_worker_deferred(pending))

    def _start_refinement_worker_deferred(self, request: tuple[int, list[str], int]) -> None:
        """遅延起動された pending refinement 要求を、発火時点の状態で再検証して起動する (#1206)。

        Args:
            request: (image_id, tags, generation) の評価要求。
        """
        if self._closing or self._refinement_inflight_id is not None:
            return  # shutdown 済み、または新しい要求が既に起動済み → stale pending は破棄
        if request[0] != self.current_image_id or not self._current_tag_canonicals:
            return
        self._start_refinement_worker(request)
    ...
    def shutdown(self) -> None:
        ...
        起動してしまう再入を防ぐ (#1024)。_closing は既に予約済みの遅延起動
    ...
    @Slot()
    def refresh_tag_metadata(self) -> None:
        """表示中画像のタグ付随メタデータ (翻訳/使用頻度/type) を tag DB から再取得する (#1205)。

        外部プロセス (CLI の翻訳追加等) の書き込みは reader からは読めるが (#1172)、
        再取得のトリガーが画像再選択しか無かった。手動ボタン・ウィンドウフォーカス復帰・
        user DB 変更検知の 3 経路が本メソッドへ合流する。single-flight (#1024/#1046) が
        実行中 worker の置換を処理するため多重実行にはならないが、トリガー連打の無駄な
        キャンセル/再起動を避けるため 1 秒未満の再入は無視する。
        """
        if self.current_image_id is None or not self._current_tags_list:
            return
        if self._refresh_rate_limiter.isValid() and self._refresh_rate_limiter.elapsed() < 1000:
            return
    ...
    def _poll_user_db_change(self) -> None:
        ...
        2 秒間隔の os.stat 1 回のみ (DB 接続は張らない)。初回観測はベースライン記録に
        とどめ、再取得は署名が変化したときだけ行う。tag DB 未初期化・ファイル欠如は
        黙ってスキップする (次回 poll で再確認)。
        """
        ...
        if self._user_db_signature is None:
