...
class SelectedImageDetailsWidget(QWidget):
    ...
    def set_tag_management_service(self, service: "TagManagementService") -> None:
        ...
        type 補正 (`tag_metadata_edit_requested`) を受けて canonical→tag_id を解決し、
        user DB overlay へ書き込む。保存先は image DB 系 (この画像の soft-reject 等) とは
        独立で、canonical 主キーで全画像に反映される。
    ...
    def _on_translation_delete(self, canonical: str, language: str, translation: str) -> None:
        ...
        保存先は image DB 系と独立 (canonical 主キー・画像 ID 非依存)。書き込み後は
        翻訳追加/主訳変更と同じ後処理 (再ベースライン + refinement キャッシュ無効化 +
        言語一覧再取得 + reload) を行う。
        """
        service = self._tag_management_service
        if service is None:
        ...
        if tag_id is None:
            ...
            return
        service.delete_translation(tag_id, language, translation)
        # widget 自身の user DB 書き込みを poll が外部変更と誤検知してカスケードするのを防ぐ (#1225)
        self._rebaseline_user_db_signature()
