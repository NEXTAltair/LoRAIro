...
class _VerticalHugScrollArea(QScrollArea):
    """縦はコンテンツ自然高さ・横はビューポート幅に追従するスクロールエリア (#831)。

    ``widgetResizable=True`` だとコンテンツがビューポートより短いとき widget が
    ビューポート高さまで引き伸ばされ、末尾に長い余白が残る。本クラスは widget の
    サイズを明示管理し、横はビューポート幅、縦はコンテンツの自然高さ (sizeHint) に
    する。短ければ上詰めで残りはスクロール背景、長ければ従来通り縦スクロールになる。
    """
    ...
    def sync_content_size(self) -> None:
        """inner widget を 横=ビューポート幅 / 縦=コンテンツ自然高さ に合わせる。

        折り返し (FlowLayout 等) の高さは幅に依存するため、先に幅を適用して
        レイアウトを reflow させてから、その幅での sizeHint 高さを反映する
        (一度の呼び出しで高さが過小にならないように)。
        """
        content = self.widget()
        if content is None:
            return
        width = self.viewport().width()
        # 1) 幅をビューポートに合わせて reflow
        content.resize(width, content.height())
        content.adjustSize()
        # 2) その幅での必要高さを反映 (短ければハグ、長ければ縦スクロール)。
        #    sizeHint は折り返しレイアウトで過小になり得るため、固定高さ子 widget の
        #    最小高さを積む minimumSizeHint を floor にしてコンテンツのクリップを防ぐ。
        needed = max(content.sizeHint().height(), content.minimumSizeHint().height())
...
class SelectedImageDetailsWidget(QWidget):
    ...
    def _apply_readable_layout(self) -> None:
        ...
        # #831: コンテンツがビューポートより短いときに末尾へ長い余白を作らない。
        # _VerticalHugScrollArea が widget を自然高さ (上詰め) に保ち、余剰は
        # スクロール背景になる (末尾 stretch spacer は不要)。

        scroll_area = _VerticalHugScrollArea(self)
        scroll_area.setFrameShape(QFrame.Shape.NoFrame)
        # 横はビューポート幅に追従させ縦はコンテンツ自然高さにするため widgetResizable=False。
        # 横スクロールバーは出さない (幅は sync_content_size でビューポートに合わせる)。
    ...
    def _toggle_image_info_section(self, expanded: bool) -> None:
        ...
        # 画像情報の開閉で高さが変わるためスクロール内容を再同期する (#831)
    ...
    def _on_annotation_data_loaded(self) -> None:
        ...
        タグ/キャプションの高さ確定後にスクロール内容の高さを再同期する (#831)。
        """
    ...
    def _sync_content_scroll(self) -> None:
        """コンテンツ更新後にスクロール内容の高さを再同期する (#831)。

        タグ表などの固定高さはレイアウト確定後に決まるため、即時同期に加えて
        イベントループ確定後の遅延同期も行い、高さの取りこぼし (クリップ) を防ぐ。
        """
        scroll = getattr(self, "_content_scroll_area", None)
        if scroll is None:
            return
