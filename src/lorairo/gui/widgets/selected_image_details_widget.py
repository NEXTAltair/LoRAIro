...
# ADR 0068 Phase 2: 詳細表示は基準 format (danbooru) の canonical で preferred 解決する
_DISPLAY_TAG_FORMAT = "danbooru"
...
class SelectedImageDetailsWidget(QWidget):
    ...
    def __init__(
        ...
        # ADR 0068 Phase 2: 整形タグ -> canonical の変換キャッシュ (同一入力の再変換を避ける)
    ...
    def set_merged_reader(self, reader: "MergedTagReader | None") -> None:
        ...
        # reader が変わると canonical 解決結果も変わるためキャッシュを破棄する
    ...
    def _canonicalize_tag(self, tag: str) -> str:
        """整形済みタグを基準 format (danbooru) の canonical へ解決する (キャッシュ付き)。

        alias->preferred 解決は format 依存のため、表示時に基準 format で都度解決する
        (ADR 0068)。reader 未設定 (外部 tag_db 不在) の場合は整形済み文字列をそのまま返す。

        Args:
            tag: 整形済みタグ文字列 (preferred 未解決)。

        Returns:
            基準 format の canonical タグ。未登録などで解決できない場合は原文。
        """
        if not tag or self._merged_reader is None:
            return tag

        cached = self._canonical_cache.get(tag)
        if cached is not None:
            return cached

        converted = convert_tags(self._merged_reader, tag, _DISPLAY_TAG_FORMAT)
        # 未登録の複数語タグは convert_tags が単語単位へ分割するため、その場合は原文を保持する
        canonical = converted if converted and "," not in converted else tag
        self._canonical_cache[tag] = canonical
        return canonical

    def _canonicalize_tags_for_display(self, tags: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """タグ詳細リストの ``tag`` を canonical へ解決した新しいリストを返す。

        tag_id などの付随情報は保持し、``tag`` のみ canonical 化する。reader 未設定時は
        入力をそのまま返す (graceful degradation)。

        Args:
            tags: Repository 由来のタグ詳細辞書リスト。

        Returns:
            canonical 化済みのタグ詳細辞書リスト。
        """
        if self._merged_reader is None or not tags:
            return tags

        canonical_tags: list[dict[str, Any]] = []
        for tag_dict in tags:
            original = tag_dict.get("tag", "")
            canonical = self._canonicalize_tag(original)
            if canonical == original:
                canonical_tags.append(tag_dict)
            else:
                updated = dict(tag_dict)
                updated["tag"] = canonical
                canonical_tags.append(updated)
    ...
    def _build_image_details_from_metadata(self, metadata: dict[str, Any]) -> ImageDetails:
        ...
        # ADR 0068 Phase 2: 表示時に基準 format (danbooru) の canonical へ preferred 解決する
        # (meta タグは表示には残す。除外は export 時のみ)
        tags_list = self._canonicalize_tags_for_display(tags_list)
        if self._merged_reader is not None and tags_list:
            tags_text = ", ".join(tag_dict["tag"] for tag_dict in tags_list if tag_dict.get("tag"))
