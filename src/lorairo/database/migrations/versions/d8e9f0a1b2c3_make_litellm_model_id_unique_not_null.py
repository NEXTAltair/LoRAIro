...
1. Step A: `name UNIQUE` 制約を data backfill 前に drop (Step 2 の name 上書き衝突対策)
2. 既存データの backfill:
   - Step 0: 旧 sync が `name == litellm_model_id` の bare 名で書き込んでいた行を NULL に戻す
   - Step 1: MANUAL_EDIT 行に sentinel `__manual_edit__` を設定
   - Step 2: スラッシュ込み name 行 (例: `OpenAI/gpt-4o`) を最初の `/` で分離し、provider を lowercase 化
   - Step 3: 既知 provider (lower 比較で openai/anthropic/openrouter/google→gemini/gemini) の litellm_model_id を `provider || '/' || name` で正規化構築
   - Step 4: 残存 NULL 行は `__legacy_<id>__` sentinel で fallback
   - Step 5: litellm_model_id 重複行を dedup (最古 id のみ正規 ID 保持)
3. Step B: schema 変更 (`litellm_model_id` を NOT NULL + UNIQUE 化)
...
def upgrade() -> None:
    ...
    # 2. スラッシュ込み name の name/provider 分離 (provider は lowercase 化)
    # 例: name='OpenAI/gpt-4o' → provider='openai', name='gpt-4o'
    # 例: name='openrouter/openai/gpt-4o' → provider='openrouter', name='openai/gpt-4o'
    # (旧実装で Step 2 として slash 込み name を litellm_model_id に verbatim コピー
    #  していたが、`OpenAI/gpt-4o` のような大文字混入をそのまま残してしまい、後の
    #  case-sensitive lookup で正規 `openai/gpt-4o` と一致せず重複登録が起きる
    #  PR #242 P2 指摘の問題があった。Step 2 = 分離のみに統合し、litellm_model_id は
    #  Step 4 で正規化済み provider/name から再構築する)
    connection.execute(
        sa.text(
            "UPDATE models "
    ...
    # 3. 既知 provider (case-insensitive) の litellm_model_id を LiteLLM 規約で構築する。
    # Step 2 で slash 込み name は分離済みのため、ここで処理する行は全て bare 形式
    # (`name='gpt-4o', provider='openai'`)。provider 値も LiteLLM 規約 (lowercase +
    # Google → gemini マップ) に整合させた上で、`litellm_model_id = provider || '/' || name`
    # で construct する。
    # `OpenAI/gpt-4o` のような大文字混入も Step 2 の分離で `provider='openai'` に
    # lowercase 化済みのため、ここで構築される litellm_model_id は常に正規 (LiteLLM SSoT) 形式。
    ...
    # 4. 残存 NULL 行 (ローカル ML モデル / 不明 provider / 空文字 / `xinntao` 等) →
    ...
    # 5. `litellm_model_id` 値の重複を解消する (UNIQUE 付与前の dedup)。
    # 旧 DB は `name UNIQUE` だったため、`name='openai/gpt-4o'` (slash 形式) と
    # `name='gpt-4o', provider='openai'` (bare 形式) が共存しうる。Step 2/3 で両行が
