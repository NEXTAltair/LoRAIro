...
down_revision: str | None = "469833dd8bda"
...
def upgrade() -> None:
    ...
    op.execute("UPDATE model_types SET name = 'multimodal' WHERE name = 'llm'")


def downgrade() -> None:
    """Revert ModelType name from 'multimodal' back to 'llm'."""
    # Revert model type name
    op.execute("UPDATE model_types SET name = 'llm' WHERE name = 'multimodal'")
