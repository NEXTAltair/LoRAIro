...
def register_images(
    ...
    if project_name:
        ...
        return _register_into_db(container.db_manager, container.file_system_manager, image_files)
...
def _register_into_db(
    ...
    マッピングするだけで、別版 (variant) を含む全経路統一の統計を返す。重複は新規行を
    作らず常に skipped に集計される (ADR 0061: 重複は既存へ寄せる)。
