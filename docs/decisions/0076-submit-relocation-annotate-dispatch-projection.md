...
- **async dispatch の submit ループは GUI スレッドから外す (ADR 0044)**。`submit_images()` は DB / filesystem / network I/O を行うため、現 `ProviderBatchJobWidget` は submit を `_ProviderBatchSubmitWorker` (worker thread) へ移し、`_set_submit_button_busy` で busy/再入ガードしている。新 `RunOptions` 経路を run ボタン / MainWindow slot から直接呼ぶと、複数モデル射影で Qt UI が固まり duplicate-submission race が再発する。新経路も worker thread + busy/再入ガード (ADR 0044) を引き継ぐ。
...
- **非 batch-capable な選択モデルを黙って落とさない**。ユーザーが local / OpenRouter 経路 / その他 batch 非対応モデルを選んだ状態で async Batch API dispatch を選ぶと、射影は batch-capable 部分集合だけ job 化する。ADR 0075 は選択モデル集合を期待出力・コストの SSoT とするため、黙殺は「選んだのに走らない」を招く。射影を実行計画として確定する前に、**(a) 部分射影を拒否する、または (b) 非 batch 残余を明示的に別経路へ回す/ledger で報告する**のいずれかを必須とする。
...
- **processed/resized 画像パスを射影に運ぶ (ADR 0064)**。staging が processed/resized パスで構成されている場合でも、`submit_images` に `image_ids` だけ渡すと `ProviderBatchWorkflowService` が `get_images_metadata_batch(..., resolution=0)` = original の `stored_image_path` にフォールバックし、ADR 0064 の processed-image safety を迂回して巨大な original をアップロードし得る。射影契約は staged の processed パス (`image_paths` override) もしくは明示的な resolution override を運ぶ。
- **moderation preflight は「送信ゲート」であって RATING 出力の派生ではない** (ADR 0070)。未評価画像に対する WebAPI 送信の事前判定 (fail-closed) なので、射影は **画像の rating / 送信可否から preflight 要否を決める**。RATING 出力ステージが選択されているか否かでは決めない (tags/caption/score のみ要求 + 未評価画像でも preflight を落とさない)。
- **preflight submit の model は射影が `openai/omni-moderation-latest` を自前解決する**。preflight job の model はユーザー選択モデル集合の一員ではない。未評価画像があり選択が通常 annotation モデルだけでも、射影は登録済み omni-moderation の `Model` 行を解決/確保して `rating_preflight` submit を合成する。これを契約に含めないと「選択 annotation モデルだけ射影 → moderation job 不在」または「`_validate_submit_task` が `openai/omni-moderation-*` 必須で reject」になる。moderation `Model` 行の解決/確保 (無ければ登録) を dispatch 契約に含める。ただし ADR 0070 line 30 は **Provider Batch の自動2段オーケストレーションを対象外**とする立場なので、async batch の自動 preflight 連鎖は本 ADR では契約 (model_id 同伴・送信ゲート由来) のみ定義し、自動2段の実装は ADR 0070 の deferral を引き継ぐフォローアップとする (bulk pre-moderation の manual rating_preflight task は CLI に残る、§3 参照)。
