...
`export create` も同じ 500 キャップ対象とする。export は coherent なデータセット成果物を生成し、特に json
フォーマットは `metadata.json` を追記でなく上書きするため、「500 ずつ分割して再実行」する pagination
workaround は成果物を壊す。よって export に分割実行手段は提供せず、500 超は他のバッチ操作と同様
`INVALID_INPUT` で弾き、フィルタの絞り込みを促す。
...
tag-db と共有する安定コア 11 種に、AI 推論ドメイン固有の 3 種を加える。各コードに `retryable` /
`user_action_required` フラグを定義し、エージェントが message 文字列をパースせずプログラムで分岐できる。

**共有コア 11 種** (tag-db 整合): `INVALID_INPUT` / `VALIDATION_FAILED` / `PRECONDITION_FAILED` /
`NOT_FOUND` / `ALREADY_EXISTS` / `CONFLICT` / `IO_ERROR` / `NETWORK_ERROR` / `DB_ERROR` / `TIMEOUT` /
`INTERNAL_ERROR`

**LoRAIro 拡張 3 種**:

| コード | retryable | user_action | エージェント反応 |
|---|---|---|---|
| `RESOURCE_EXHAUSTED` | true | false | batch_size を下げて再試行 |
| `AUTH_ERROR` | false | true | API キーを設定 |
| `RATE_LIMITED` | true | false | backoff して再試行 (`details.retry_after`) |
...
→ `RESOURCE_EXHAUSTED`、`anthropic.` / `openai.` の `AuthenticationError` → `AUTH_ERROR`、
同 `RateLimitError` → `RATE_LIMITED`。
...
- Click usage error → `INVALID_INPUT` の `error` 行 + exit 2
...
- **500 ハードキャップ**: GUI の staging (最大 500 枚) と挙動を統一する。処理前に弾くため副作用ゼロで
  fail fast。自動切り詰め + ページングよりも、利用者に絞り込みを促す方がデータ欠落リスクがない。
  実運用上、1 回の学習データセットが 500 枚を超えることは稀で、この上限が bulk 用途を実質的に阻害しない。
  `export create` も対象に含めるのは、export の json 成果物が `metadata.json` 上書き方式で、分割再実行に
  よる pagination が成果物を破壊するため。export には分割手段を設けず、500 超はフィルタ絞り込みで対応する。
...
- kind スキーマとエラーコードの具体定義は `docs/cli.md` (新設) を SSoT とし、人間とエージェントの双方が
  参照できる契約文書とする。
...
- ADR 0053 (CLI Streaming Annotation Memory-Bounded Contract) — `RESOURCE_EXHAUSTED` / streaming と整合
