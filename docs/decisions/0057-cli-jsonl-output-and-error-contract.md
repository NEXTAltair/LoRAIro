...
- **§3 cap コード**: 処理系バッチ (annotate / export / images update) の 500 超過の弾きコードを
  `INVALID_INPUT` から `RESULT_SET_TOO_LARGE` に統一する。recourse (sharding 等) の記述は不変。
- **§3 read/list の扱い**: 「read/list はキャップ対象外、pagination は #639」という委譲を ADR 0060 が
  具体化する。read/list も基本 500 で bound するが、count 既定 + fetch 明示という別形式で実現し、処理系の
  処理前ハード弾きとは UX が異なる。
...
- ADR 0060 (CLI Bounded Pagination and Count-First Contract) — read/list の bounded pagination 契約を確定し、
  本 ADR §3/§4 を amend (`RESULT_SET_TOO_LARGE` 追加・cap コード統一)
