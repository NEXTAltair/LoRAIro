...
(`ensure_ascii=False` で日本語タグを保持、`default=str` で `Path` / `datetime` / `Enum` を文字列化)。
...
```jsonc
// item — list 系コマンドの 1 レコード (N 行)。record を model_dump(mode="json") でトップレベル展開。
{"kind": "item", ...}
...
- **進捗を表す `event` kind は採用しない**。list 系は `item` がストリームされるため item 数が進捗を表し、
  バッチ操作 (annotate / export) は最終 `result` のみで足りる。進捗は §1 のとおり stderr の人間向け
  Progress バーに限定する。
