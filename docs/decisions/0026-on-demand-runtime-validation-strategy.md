...
GitHub Actions に実 model DL / 実 WebAPI 呼び出しを伴う workflow は追加しない。`workflow_dispatch` / `schedule` を含め、CI 経由での実 runtime 実行は禁止する (本 ADR amendment 2026-05-18)。実 runtime validation はすべて開発者ローカルで行い、必要時に手動実行する。

LoRAIro 側 repo は実 model DL / 実 WebAPI を呼ぶ test を一切持たない。これらは image-annotator-lib repo の `tests/runtime_validation/` 配下に集約する (iam-lib ADR 0001 amended)。LoRAIro 側からは `Makefile` target (`test-runtime-local` / `test-runtime-webapi`) 経由で間接実行する。
...
| Marker | 用途 |
|---|---|
| `e2e` | deterministic local fixtures / fake backend で動く E2E (CI で実行) |
| `cli` | CLI entrypoint / subprocess / command workflow |
| `webapi` | Web API annotator 関連 test 全般 (mock 含む)。CI で実行可 |
| `calls_real_webapi` | 実 provider API key を使う on-demand validation (`webapi` の subset)。**CI 不経由、ローカル only** |
| `downloads_and_runs_model` | 実 model DL + 実推論を伴う on-demand validation。**CI 不経由、ローカル only** |
| `slow` | 通常 CI から除外する重いテスト |
| `model_factory` / `scorer` / `tagger` | model loading / inference domain の分類 |

`e2e` は「実外部依存を使う」という意味ではない。実外部依存を使う場合は `calls_real_webapi` または `downloads_and_runs_model` を使用し、CI から完全除外する。

**Marker 区別 (`webapi` vs `calls_real_webapi`)**:
- `webapi`: Web API annotator に関連する test 全般 (mock 含む)。CI で実行される。
- `calls_real_webapi`: 実 API key で実リクエストを送信する test。`webapi` の subset として併用可 (`@pytest.mark.webapi @pytest.mark.calls_real_webapi`)。CI からは除外される。

**Marker 組み合わせ運用**: `calls_real_webapi` と `downloads_and_runs_model` は排他ではない。LoRAIro adapter 経由で実 model + 実 WebAPI 両方を 1 test で検証する場合、両 marker を併用可能。filter 上は `not (downloads_and_runs_model or calls_real_webapi)` で CI 除外される。
...
- 実 provider API / 実 model DL を伴う test は LoRAIro 側 repo に置かない。iam-lib repo の `tests/runtime_validation/` 配下 (ADR 0001 amended) に集約し、LoRAIro 側からは `Makefile` target で間接実行する。
- model download / 実推論 / 長時間実行を伴う検証は LoRAIro 側 E2E に含めない。必要な場合は image-annotator-lib 側 ADR 0001 (amended) に従って同リポジトリで扱う。
- CLI full workflow E2E は、可能な限り fake backend / fixture を使って `tests/integration/` に置く。
- BDD はユーザー仕様として読み下したいシナリオに限定し、E2E の標準配置にはしない。
- 実 API / 実モデル validation を CI (scheduled / workflow_dispatch / mandatory) に昇格することは禁止する (本 ADR amendment 2026-05-18)。再導入する場合は本 ADR と iam-lib ADR 0001 の両方を破棄し、新規 ADR で再設計する。
