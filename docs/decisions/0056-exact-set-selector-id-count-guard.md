...
**exact-set selector に「数字ガード」(500) を、`get_images_by_ids` に「バインド安全
ガード」(BATCH_CHUNK_SIZE) を導入する。metadata 取得ヘルパーのチャンク分割は行わない。
count-only の直接 COUNT 化も行わない (対象外とする)。**
...
### 1b. `get_images_by_ids` はバインド安全ガードで弾く (関連経路)

- `get_images_by_ids` (`image.py`, error workflow 用の別 ID 経路) も全 ID を一括 IN で
  構築するため、同じバインド上限リスクを持つ。本 ADR で併せてガードする。
- **ただし閾値は 500 ではなく `BATCH_CHUNK_SIZE` (= 15000)** とする。この経路は
  エクスポート集合ではなく、呼び出し元 (`db_manager.py` の error workflow) が
  **未解決アノテーションエラーの全件**を渡す。大規模バッチ失敗時に 500 を超え得るため、
  500 でガードすると正当なエラー復旧 fetch を弾く回帰になる。意味的に「エクスポート集合
  上限 (500)」ではなく「バインド安全 (BATCH_CHUNK_SIZE)」が正しい。
- `len(image_ids) > BATCH_CHUNK_SIZE` で `ValueError` を送出する。現状すでに単一 IN なので
  クエリ構造の変更はなく、SQLite 例外を明確な契約違反エラーに変換するのみ。
...
- ◎ `get_images_by_ids` も同時にバインド安全ガードを得る (error workflow の隠れたクラッシュ
  リスクを明確な `ValueError` に変換)。
...
- △ `get_images_by_ids` は 2 つの閾値を持つことになる (exact-set=500 / by_ids=15000)。
  意味 (エクスポート集合 vs バインド安全) が異なるため意図的に分離するが、「なぜ違うのか」を
  本 ADR で明示しておく必要がある。
...
- [ ] `_fetch_images_by_exact_ids`: `len > 500` で `ValueError` の回帰テストを追加する。
- [ ] `get_images_by_ids`: `len > BATCH_CHUNK_SIZE` で `ValueError` の回帰テストを追加する。
- [ ] `EXACT_SET_MAX_IDS == StagingWidget.MAX_STAGING_IMAGES` の drift 防止 assert を追加する。
