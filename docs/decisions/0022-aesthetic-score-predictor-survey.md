...
2. ~~**マルチモデル評価の集約方針 / 尺度統一**~~ **(解決済み: #626 / ADR 0029)**:
   モデル間でスケールが異なる数値スコアを手動スライダーと同じ 0.0-10.0 表示尺度へ
   統一する方針が確定した。`src/lorairo/domain/score_scaler.py` が各 scorer の positive
   key (`higher_is_better=True`) を Animagine 由来しきい値を knot にした連続・区分線形
   補間で 0-10 化し、手動値があれば最優先、無ければ各 scorer の連続 0-10 の平均を表示
   スコア (`score_value`) とする。離散の品質ティア badge (ADR 0029
   `compute_quality_summary`) は併存し、連続 display score と役割分担する。lib 側の
   `ScoreScale` (range / higher_is_better) メタデータ安定化は iam-lib#144 で扱う。
3. **VisionReward 統合時の `UnifiedAnnotationResult` 拡張**: 多次元評価を保持する `aspects: dict[str, float]`
   フィールド追加の要否 (現時点では `raw_output` 任意辞書で対応可能)

## Related

- [ADR 0029 — Unified Dataset Quality Tier](0029-unified-dataset-quality-tier.md): tier badge (離散) と display score (連続) の役割分担
- [Issue #626](https://github.com/NEXTAltair/LoRAIro/issues/626): AI 数値スコアの表示尺度統一 (consumer 側)
- [image-annotator-lib#144](https://github.com/NEXTAltair/image-annotator-lib/issues/144): scorer `ScoreScale` メタデータ安定化 (provider 側)
