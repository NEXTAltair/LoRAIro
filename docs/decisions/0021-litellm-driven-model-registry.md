...
ADR 0023 により、本 ADR のうち WebAPI model registry の運用方式は一部 supersede された。
現在有効な決定は ADR 0023 を優先する。

Supersede された範囲:

- `available_api_models.toml` を LiteLLM 出力キャッシュとして継続する方針
- `available_api_models.toml` / `_WEBAPI_MODEL_METADATA` を WebAPI metadata 正本にする方針
- WebAPI 用 `user_config.toml` override を残す方針
- TTL refresh / 手動 refresh / OpenRouter fallback fetch / force refresh / regenerate による追従方針
- `supports_response_schema` / structured output 対応を WebAPI model filtering の主要条件にする方針

現在の方針:

- WebAPI model registry は LiteLLM 同梱 DB を runtime 評価する
- `available_api_models.toml` は廃止する
- WebAPI 用 user TOML override は廃止する
- OpenRouter fallback fetch 等のネットワーク取得経路は削除する
- model filtering は vision + function/tool calling 対応を主要条件にする

ローカル ML モデルを `annotator_config.toml` / `user_config.toml` で扱う方針、および
LiteLLM を WebAPI discovery / capability metadata の source として使う大枠は引き続き有効。
...
3. `available_api_models.toml` は **LiteLLM 出力のキャッシュ** として継続
   - **Superseded by ADR 0023**: 現在は廃止し、LiteLLM 同梱 DB を runtime 評価する
...
4. WebAPI モデル metadata の正本は **`available_api_models.toml` / `_WEBAPI_MODEL_METADATA`**
   に集約する
   - **Superseded by ADR 0023**: 現在は runtime LiteLLM call を正本にする
   ...
   - **Superseded by ADR 0023**: 現在の主要条件は vision + function/tool calling 対応。
     `supports_response_schema` は NativeOutput 最適化用 metadata として扱う
5. WebAPI モデルの user TOML は **モデル定義ではなく最小限の実行時 override** とする
   - **Superseded by ADR 0023**: WebAPI 用 user TOML override は廃止する
...
7. **追従の三重化**で更新を確実にする
   - **Superseded by ADR 0023**: runtime LiteLLM local DB call に移行するため、cache refresh
     機構は削除対象
   - TTL ベースの起動時 background refresh（既定 7 日）
   - GUI/CLI からの手動更新トリガー
   - CI による週次 LiteLLM 更新 + 自動 PR 作成
8. **OpenRouter API は補助 fallback** として残す
   - **Superseded by ADR 0023**: OpenRouter fallback fetch は削除対象
...
- **対象外**:
  ...
  - PydanticAI 推論パイプライン → 既存実装維持
    - **Superseded by ADR 0023**: WebAPI 推論境界は PydanticAI native provider/model に整理する
...
- ネットワーク失敗時も既存 toml キャッシュで fallback 可能
  - **Superseded by ADR 0023**: 現在は WebAPI toml キャッシュを廃止し、起動時ネットワーク取得を
    行わない
...
**Superseded by ADR 0023**: `available_api_models.toml` refresh 系は廃止し、LiteLLM 同梱 DB の
runtime 評価に移行する。この節は ADR 0021 採用時点の理由としてのみ残す。
...
**Superseded by ADR 0023**: WebAPI 用 user TOML override は廃止する。この節は
ADR 0021 採用時点の理由としてのみ残す。
...
- **LiteLLM の DB 精度に依存**: 誤判定時は OpenRouter フィルタとの AND 評価で厳格モードを実装
  - **Superseded by ADR 0023**: OpenRouter fallback fetch は廃止し、LiteLLM 同梱 DB と
    PydanticAI 実行時エラーで扱う
...
2. ISSUE B で LiteLLM 依存追加と discovery 移行（fallback 残す）
   - **Superseded by ADR 0023**: fallback fetch は残さない
...
8. ISSUE E〜G で TTL/UI/CI 等の追従機構を順次追加
   - **Superseded by ADR 0023**: `available_api_models.toml` refresh 系は実装しない
9. リリースノートで移行手順を明示
