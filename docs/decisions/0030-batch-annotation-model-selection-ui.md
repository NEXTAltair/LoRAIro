...
> **将来 [ADR 0075](0075-annotation-pipeline-composition-domain-model.md) が supersede 予定 (Proposed)**:
> 本 ADR の「二段階フィルタ」モデル選択 UI と `environment = Literal["api","local"]`
> (環境未選択状態なし) は、per-stage ピッカーへの移行 (#741 / #839 / #845) で実態を
> 離れた。後継の ADR 0075 が **Accepted になった時点で** 本 ADR を Superseded に切り替える。
> それまでは本 ADR を accepted な正準として残す (空白期間を作らない)。
