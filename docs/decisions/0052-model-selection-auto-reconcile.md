...
複数の `ModelSelectionWidget` が生成されても二重 sync は避けられる。起動直後の初回 widget
表示では短時間だけ refresh progress が表示される可能性があるが、UI スレッドはブロックしない。
version-change 検知や永続的な sync 履歴は今回導入しない。
