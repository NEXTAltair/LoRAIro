...
## ADR 0057 への Amendment (本 ADR の Accept で適用済み)

本 ADR の Accept 昇格に伴い、ADR 0057 §2 / §3 / §4 / §6 を以下のとおり改定した (適用済み)。改定内容の SSoT は
本 ADR であり、0057 本体・README への反映は本 ADR の Accept (= 本 PR の merge) と同一コミットで行った
(Proposed の間は確定済み 0057 を編集せず、未確定変更の混入を避けた):
