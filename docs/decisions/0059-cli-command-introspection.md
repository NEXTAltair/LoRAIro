---
type: ADR
title: CLI Command Introspection Contract
status: Accepted
...
コマンド仕様を専用の `tool` / `model` 行の JSONL で晒すことでこの課題を解決済み。LoRAIro にも同等の
introspection を導入する (epic #634) が、後述のとおり wire 表現は tag-db の専用 kind ではなく ADR 0057 の
既存 `item` kind に乗せる (LoRAIro CLI は MCP サーバではなくエージェントフレンドリー CLI であり、MCP 的な
formal tool 定義の専用 kind を wire に増やす必然がないため)。
...
### 1. `list-commands` / `describe <command>` を追加し、ADR 0057 の既存 kind に乗せる

`lorairo-cli list-commands` と `lorairo-cli describe <command>` を追加する。**ADR 0057 が定める kind は
`item` / `result` / `error` の 3 種に閉じており (0057 §2 は進捗用 `event` の追加すら明示的に拒否している)、
本 ADR はこの enum を拡張しない**。introspection は専用 kind を増やさず、既存の `item` (+ 終端 `result`) で
表現する。

これは **LoRAIro CLI がエージェントフレンドリー CLI であって MCP サーバではない**ことに由来する判断。MCP の
ように formal な tool 定義 (専用の tool/model スキーマ kind) を wire 契約に持ち込む必然はなく、
`describe` / `list-commands` は「コマンドを列挙する list 系コマンド」にすぎないため、0057 の「list 系 =
`item` を N 行 + 終端 `result`」パターンにそのまま乗る。tag-db ADR 0005 は専用 `tool` / `model` 行を使うが、
LoRAIro は 0057 の kind enum 純度を保つためこの点で意図的に分岐する。

- `list-commands` → コマンドごとに `kind:"item"` を 1 行 (`type:"tool"`、メタ: 名前・`read_only`・
  `side_effects`) + 終端 `kind:"result"` (件数)。
- `describe <command>` → 対象コマンドの `kind:"item"` 行を並べる:
  - `type:"tool"` の `item` 1 行 (コマンドのメタ)
  - `type:"model"` の `item` 行 (入力 `role:"input"` / 出力 `role:"output"` / エラー `role:"error"` の
    フィールド)
  - 終端 `kind:"result"`
- **`tool` か `model` かは `item` payload 内の `type` フィールドで区別する** (`kind` は `item` のまま)。
...
`describe` のスキーマ表現を 2 モードとする。**いずれも `kind:"item"` 行に載せ、ADR 0057 の全行 JSONL 不変
条件を保つ** (tag-db ADR 0005 のような stdout への非 JSON `#` 行は出さない):

- **`compact` (既定)**: フィールドを簡易型表記 (`str (required)` / `int>=1?` / `list[str]?` /
  `bool=true`) で `type:"model"` の `item` 行に載せる。入れ子モデルは名前参照。人間にもエージェントにも読める
  既定形で、ADR 0057 の「行を浅く保つ」方針と整合する。
- **`json_schema`**: `model_json_schema()` の完全版を `type:"schema"` の `item` 行の payload に **1 モデル =
  1 item 行**として埋め込む。生スキーマを stdout に直接書かず `item` に包むことで、JSON Schema が深くネスト
  していても **各行は単一の有効な JSON オブジェクト**であり続け、0057 の全行 JSONL 契約を破らない。
...
- **introspection を 0057 の既存 kind に乗せる (専用 kind を足さない)**: LoRAIro CLI は MCP サーバではなく
  エージェントフレンドリー CLI なので、MCP 的な formal tool 定義 (専用 tool/model kind) を wire に持ち込む
  必然がない。`describe` / `list-commands` は list 系コマンドそのものなので 0057 の `item` + `result` に
  素直に乗り、0057 が意図的に閉じた kind enum (item/result/error) を一切拡張せずに済む。エージェントは
  introspection も実行結果も同じ JSONL パーサ・同じ kind 集合で読める。
- **compact 既定 + json_schema**: compact は生 JSON Schema より人間が読みやすくエージェントも解釈できる。
  完全な機械可読が要る場面 (検索スキーマの厳密な制約読み取り等) のために json_schema を用意し、深くネスト
  した生スキーマは `type:"schema"` の `item` 行に 1 モデル 1 行で包んで全行 JSONL を保つ。
...
- **ADR 0057 の kind enum (item/result/error) は拡張しない。** introspection は専用 kind を足さず `item`
  (+ 終端 `result`) で表現するため、0057 を amend する必要も、stdout に非 JSON 行を出す例外も生じない。
  `tool` / `model` / `schema` の区別は `item` payload の `type` フィールドで持つ (docs/cli.md に明記する)。
...
- tag-db ADR 0005 (CLI Command Introspection Contract) — 思想の参照元 (LoRAIro は Typer / ネスト / 横断検索
  契約に加え、専用 `tool` / `model` kind を足さず 0057 の `item` kind に乗せる点で wire 表現が異なる)