...
soft-rejected には**2つの提示**を持つ。どちらも `rejected_at` をセットし DB 行は残す。両者は「表示／非表示」を区別する表示状態で分ける（永続化の具体は #987 で確定）:

| 操作 | DB | 提示 | 復活 |
|---|---|---|---|
| **無効化**（chip クリック） | `rejected_at` セット | 破線 chip でインライン表示を継続 | chip 再クリックでトグル復活 |
| **外す**（✕ ボタン） | `rejected_at` セット | パネルから非表示 | **先送り**（隠したものを戻す UI は作らない） |

chip 操作割当（プロト準拠・現状の #814 と両立）:

- 単クリック = 無効化⇄復活トグル（破線インライン）
- ✕ = この画像から外す（非表示・復活先送り）
- Ctrl+クリック = コピー選択（#814）
- ⋯ ボタン（inline）/ 右クリック = タグ情報メニュー（翻訳追加 / タグ情報を編集 / 使用頻度を見る / 警告を無視）

別枠「復活」セクションは作らない（インライン破線のクリック復活で代替し、✕ で隠したものの復活は先送り）。`tag_reject_requested` / `tag_restore_requested` は両提示で共通（保存先は同じ soft-reject）で、非表示か破線表示かは `TagPanelWidget` の表示状態が持つ。
...
- soft-reject に「破線で残す（無効化）」「非表示にする（外す）」の2提示を持たせるため、`rejected_at` に加えた表示状態の区別が必要になる（#987 で永続化方法を確定）。✕ で隠したタグの復活 UI は先送りで、それまで隠したタグは再表示できない。
