...
- Ctrl+クリック = コピー選択（#814）
- ⋯ ボタン（inline）/ 右クリック = タグ情報メニュー（翻訳追加 / タグ情報を編集 / 使用頻度を見る / 修正候補を適用 #1007・編集モードのみ / 警告を無視）
...
> **スコープ外（別 Phase に後送り）**: ① 複数選択のバッチ操作バー（まとめて外す/無効化/復活）— #997/#1001 で実装済み。② refinement 警告の「修正候補を適用（タグ置換、`refine.suggest`）」— **#1007 で実装済み**: chip 右クリックメニューに `correction_candidate` の候補を「修正候補を適用」として並べ（編集モードかつアクティブ chip のみ）、`tag_replace_requested(from, to)` → 親が `replace_tag_for_images_batch`（置換元 `reject_reason='replaced'`、Issue #1003）へ dispatch し reload 1回。対象はこの画像のみ（全画像一括は CLI `lorairo tags replace` の領分）で、バッチ適用（複数警告まとめて置換）と自動適用は引き続き対象外。③ ✕ で隠したタグの復活 UI — 引き続き先送り。
