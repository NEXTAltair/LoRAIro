...
- ドラフト PR として作成した場合でも、ローカル検証済みでレビュー可能なら同じセッションで
  Ready for review に切り替えてから保守ループを開始する
- PR 作成時は、ユーザーが明示的に draft を求めた場合や実装がまだレビュー不能な場合を除き
  `--draft` を使わない
- PR 作成直後に `gh pr view "$PR" --json isDraft -q .isDraft` で draft 状態を確認し、
  draft なら `gh pr ready "$PR"` を実行して再確認する。再確認後も draft の場合は
  auto-merge setup に進まず、明確なエラーとして停止する
- ユーザーが明示的にドラフト維持を求めた場合、検証未完了・失敗中の場合、またはスコープが
  レビュー可能ではない場合のみドラフトのまま停止し、理由を報告する
...
- PR が draft ではない
- PR 作成直後の no-draft postcondition が成功している
