...
派生出力は **既定で採用** される。compose で opt-out させない理由は、推論が固定
スキーマで全種類を返す以上、UI で隠してもデータは生成・保存されるため (表示と実体の
乖離を避ける)。

不要な派生出力の事後除外は出力種類で **非対称** である:

- `tags` / `caption`: Results の soft-reject (ADR 0065、`rejected_at`) で per-row に外せる。
- `score`: per-row の reject 経路が現状ない (ADR 0065 / `rejected_at` は tags/caption のみ、
  `Score` テーブルに reject 列なし)。派生 score は採用されたまま残る。除外が必要になった
  場合は score の reject 経路追加が前提 (本 ADR 範囲外、follow-up)。

### 推論回数 = ユニークモデル数 × ステージング枚数

推論回数 (= 課金単位 = 推論アイテム数) は **ユニークモデル数 × ステージング枚数**。同一
モデルを複数のアノテーション種類に充当しても `litellm_model_id` で dedupe され、
コストは増えない (`InferenceLedger`)。この「推論アイテム」は ADR 0066 の Jobs lifecycle
が数える **ジョブ (Pipeline/Operation 粒度) とは別の粒度** である (下記「用語」参照)。
...
| 用語 | 意味 |
|---|---|
| ジョブ (job) | **Pipeline / Operation 粒度**の実行単位 (ADR 0034 / 0066)。Jobs lifecycle view に載る。Provider Batch は `provider_batch_jobs` |
| 推論アイテム / 推論回数 | 1 モデル × 1 画像の推論。INFERENCE LEDGER が数える単位。**ジョブとは別粒度** |
| アノテーション種類 | 出力の種類 = `tags / caption / score / rating` |
| `task_type` | provider batch の処理モード = `annotation` / `rating_preflight`。アノテーション種類とは別軸 |

- `tags/caption/score/rating` を「タスク」と呼ばない (provider batch の `task_type` と衝突する)。
- 1 モデル × 1 画像の推論を「ジョブ」と呼ばない (ADR 0066 の Pipeline/Operation 粒度の
  ジョブと衝突する)。「推論アイテム / 推論回数」を使う。
- 既存 `InferenceLedgerWidget` の表示ラベル「推論ジョブ合計」は本定義と不整合。ラベルを
  「推論回数」系へ寄せるのは follow-up とする。
...
- ADR 0030 (Batch Annotation Model Selection UI) の「二段階フィルタ」「env =
  `api`/`local` のみ」の決定は本 ADR が置き換える。ただし本 ADR が **Accepted になった
  時点で** ADR 0030 を Superseded に切り替える。それまでは ADR 0030 を accepted な正準
  として残し、per-stage セマンティクスの空白期間を作らない (#885)。
