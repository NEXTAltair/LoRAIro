...
- **代償 / 留意点**
  ...
  - **長辺解像度は DB の original 画像メタデータ (`width`/`height`) から導く**。`image_paths`
    override で original と異なる解像度の素材を投入する場合、custom_id は override の実解像度
    ではなく original 解像度基準になる。現行の submit 経路 (GUI / CLI / moderation preflight)
    は override 解像度差が custom_id 衝突を生む状況を作らない前提で運用する。override の実解像度
    を custom_id に厳密反映する必要が出た場合は、caller が override 解像度を渡す拡張で対応する
    (本 ADR の対応表構造はそのまま流用できる)。
  - **legacy JSONL import (`BatchImportService`) の fan-out は代表 image 1 件に限定する**。
    submit/import 経路は `raw_request` の `lorairo_image_ids` 対応表で全 image へ fan-out するが、
    外部生成 JSONL を取り込む legacy 経路には対応表が無い。同一 (pHash, 長辺) が複数レコードに
    重複登録されている場合、`BatchImageMatcher` は代表 (昇順先頭) を `matched` に返し、重複群は
    `ImageMatchResult.ambiguous` に保持する (取りこぼし情報を残す)。legacy 経路で全重複へ反映する
    必要が出た場合は、`BatchImportService` が `ambiguous` を消費して fan-out する拡張で対応する。
