...
provider 横断で実装済みだが、image-annotator-lib 側 provider adapter は **Anthropic のみ完成**で、
OpenAI / Google は未実装である。
...
| OpenAI direct | 完全実装 (submit を受け付ける) | **未実装** | いいえ。submit すると lib dispatch で `unsupported_provider` |
...
- **Anthropic direct route の API key を持っている** (OpenAI direct adapter は未実装のため、現時点では
  Anthropic のみが実用対象)
...
- [ ] Anthropic direct route の API key が `config/lorairo.toml` の `[api] claude_key` または
      環境変数 `ANTHROPIC_API_KEY` で設定されている
...
ADR 0038 Phase 1 MVP として CLI / GUI / Service / DB 層は OpenAI を受け付ける形で実装されているが、
image-annotator-lib 側に OpenAI Batch adapter が **存在しない**。`lorairo-cli batch submit --provider openai`
を実行すると、lib dispatch 層で `unsupported_provider` 相当の `ProviderBatchError` が
発生して exit code 1 で失敗する。

OpenAI direct route が有効化された時点で参照する setup checklist (将来形):
...
- `/v1/responses` / `/v1/chat/completions` / `/v1/embeddings` 等が対応 endpoint
...
CLI は以下のエラーで abort する:

```
Error: Google Provider Batch submit is disabled until Phase 3.
```
...
`openrouter/...` prefix の litellm model ID で submit を試みると、CLI は以下のエラーで abort する:

```
Error: Could not infer a direct Provider Batch provider for 'openrouter/...'.
...
| Resized image path の提供 | LoRAIro | `BatchSubmitItem.image_path`、長辺 512px |
...
ただし運用上の事実として、`provider_batch_jobs.raw_provider_payload` カラムに provider が返す raw
JSON 文字列が submit / status / fetch 各タイミングで保存される (privacy 注意点、§ Privacy / retention を参照)。
...
# → Error: Unsupported Provider Batch provider 'openai'. (現状の lib dispatch)
```
...
- 想定外例外: exit code 2 で `logs/lorairo.log` に stacktrace
...
### `raw_provider_payload` カラムへの保存

**重要**: `provider_batch_jobs.raw_provider_payload` カラムには submit / status / fetch 各時点で
provider が返す raw JSON が JSON-serialized 文字列として保存される。

- DB に provider 生レスポンスが残るため、SQLite ファイル (`lorairo_data/<project>/<project>.db`)
  をバックアップ / 共有する際は注意する
...
| `list_batch_capable_models()` の対象 | direct provider 全部 | Anthropic 登録 model のみ返す |
| OpenAI adapter (Phase 1 MVP) | 最初に実装 | 未実装 (CLI/GUI は受付、lib dispatch で reject) |
