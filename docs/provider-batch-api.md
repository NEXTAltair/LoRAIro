...
LoRAIro 上で job を投入、確認、キャンセル、取得、import するための運用手順をまとめる。
...
- [ ] completed 後、OpenAI 側 retention 期限内に fetch / import する運用になっている
...
- LoRAIro は library から返る normalized result を DB に保存する
...
- [ ] 24h expiration と results availability 期限内に fetch / import する運用になっている
- [ ] Zero Data Retention が必要な workflow ではない

注意:

- 24h 以内に処理されなかった item は expired として扱う
- results は provider retention 期限内に fetch する
...
- `imported` の job は二重 import しない
- provider retention 期限を過ぎると results を取得できなくなる可能性があるため、completed job は早めに fetch する
...
`image-annotator-lib` の normalized result contract を通して実行する。
...
| completed | provider results が取得可能 | retention 期限内に fetch / import する |
...
| imported | LoRAIro DB へ import 済み | 二重 import しない |

Item 単位の扱い:

- succeeded item は import 対象になる
...
- completed job は provider retention 期限内に fetch する
- fetch 済み artifacts は LoRAIro の local project data として扱い、通常の backup / purge policy に従う
...
- Provider Batch job queue は `provider_batch_jobs` / `provider_batch_items` / `provider_batch_artifacts`
  に job state を保存する
- Provider Batch job queue は `custom_id = img-{image_id}` を result 照合の SSoT とする
- Provider Batch job queue は OpenAI / Anthropic の provider 差分を `image-annotator-lib` 側で吸収する

新規 workflow では Provider Batch job queue を使う。既に取得済みの OpenAI JSONL を後から取り込む場合だけ、
legacy/manual import を使う。
