...
`.agents/skills/`: `check-existing` (実装前の既存解調査), `lorairo-repository-pattern`, `interface-design`, `lorairo-qt-widget`, `lorairo-test-generator`, `lorairo-mem` (長期記憶 + session保存), `lorairo-cli-db-access` (画像DB調査は sqlite3 直叩きでなく lorairo-cli 経由), `lazy-import-refactor`, `pr-maintainer` (PR保守ポリシー), `pr-autoloop` (PR保守ループ自走)
